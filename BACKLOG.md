...
## Sample Studio (design: docs/sample-studio/)

The custom-DirectSound-sample feature is fully designed in
`docs/sample-studio/` (CONTEXT/PLAN/FORMATS/DSP) with a 6-phase plan.
Items explicitly deferred out of even that plan:

- **16-bit .wav output toggle** for users who post-process samples in
  external tools (8-bit chosen for audition/build bit-parity).
- **Offer to install wav2agb** into legacy `.aif`/aif2pcm forks (the plan
  only refuses actionably there; FORMATS.md appendix A has the no-DSP
  `.aif` fallback design if ever wanted).
- **Sample rename/delete** management UI (creation only, for now).
